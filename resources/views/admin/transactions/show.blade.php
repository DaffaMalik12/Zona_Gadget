...
@section('header-actions')
    <a href="{{ route('admin.transactions.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:border-indigo-300 hover:text-indigo-600">
...
@section('content')
    ...
    <div class="mb-7 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
            ...
            <p class="mt-1 text-lg font-bold text-slate-800">{{ $transaction->user->name ?? $transaction->user->username }}</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 bg-slate-50/60 px-6 py-4">
            <h2 class="text-base font-bold text-slate-800">
                <i class="fas fa-shopping-bag mr-2 text-indigo-500"></i>Item Transaksi
            </h2>
                ...
                <tbody class="divide-y divide-slate-50">
                    @foreach ($transaction->transactionsItems as $index => $item)
                        <tr class="transition hover:bg-indigo-50/30">
                            ...
                            <td class="px-6 py-3.5 text-sm font-semibold text-slate-800">{{ $item->product->name }}</td>
