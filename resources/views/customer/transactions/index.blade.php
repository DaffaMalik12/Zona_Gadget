...
@section('page-subtitle', 'Riwayat transaksi pembelian Anda')

@section('header-actions')
    <a href="{{ route('customer.transactions.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-500 px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-indigo-500/20 transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-indigo-500/30">
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 bg-slate-50/60 px-6 py-4">
            <h2 class="text-base font-bold text-slate-800">
                <i class="fas fa-list-alt mr-2 text-indigo-500"></i>Riwayat Transaksi
            </h2>
                            ...
                            <tr class="transition hover:bg-indigo-50/30">
                                ...
                                <td class="px-6 py-3.5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-semibold text-slate-700">{{ \Carbon\Carbon::parse($trx->transaction_date)->format('d M Y') }}</p>
                                            <p class="text-[11px] text-slate-400">{{ \Carbon\Carbon::parse($trx->transaction_date)->format('H:i') }} WIB</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-3.5">
                                ...
                                <td class="px-6 py-3.5">
                                    <a href="{{ route('customer.transactions.show', $trx->id) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-sky-50 px-3 py-1.5 text-xs font-semibold text-sky-600 transition hover:bg-sky-100">
        ...
        @else
            <div class="py-16 text-center">
                <i class="fas fa-shopping-cart mb-3 text-4xl text-slate-300"></i>
                <h3 class="text-lg font-semibold text-slate-700">Belum Ada Transaksi</h3>
                <p class="mb-4 text-sm text-slate-500">Anda belum melakukan transaksi apapun</p>
                <a href="{{ route('customer.transactions.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700">
                    <i class="fas fa-cart-plus"></i> Buat Transaksi Pertama
                </a>
            </div>
        @endif
